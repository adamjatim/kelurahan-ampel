<div class="m-5 lg:w-2/6">
  ...
  <form>
    <div class="flex flex-col border rounded-lg slideanim">
      ...
      <div class="flex w-full px-3 py-4">
        <form action="/artikel">
          <input type="search" name="cari" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-s-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-s-gray-700  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-blue-500" placeholder="Enter search term...  " required>
  ...
  <div class="my-6 border rounded-lg slideanim">
    ...
    <div class="grid flex-rows grid-cols-2 gap-2 px-3 py-3">
      ...
      <ul class="my-auto mx-2">
        <li class="">
          <a href="/artikel?kategori={{ $category->slug }}" class="decoration-none flex-grid justify-between grid-auto-cols flex">
            <span>
              {{ $category->nama }}
            </span>
            <span>
  ...
  <div class="flex flex-col border rounded-lg slideanim">
    <div class="px-2 py-3 bg-gray-200 rounded-t-lg dark:bg-gray-700">
      <label class="mx-2 text-md font-medium text-gray-900 dark:text-white">{{ $label }}</label>
    </div>
    <div class="grid grid-col-2 gap-2 px-3 py-3">
      @foreach ($views as $view)
        <div class="flex flex-row">
          <div class="">
            <a href="/artikel/{{ $view->slug }}">
            <img src="{{ asset('images/'.$view->gambar) }}" class="h-auto max-w-40 justify-items-start rounded-lg" alt=""></a>
          </div>
          <div class="flex-initial px-4 place-content-center">
            <a href="/artikel/{{ $view->slug }}" class="font-base font-bold text-gray-800 block no-underline">
            {{ $view->judul }}</a>
            <small class="font-sm font-light">{{ $view->created_at->format('d F Y') }}</small>
          </div>
        </div>
      @endforeach
    </div>
